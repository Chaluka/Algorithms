...
class HashIterator(Generic[T]):
    ...
    def __init__(self, array:ArrayR):
        self.array = array
        self.current = 0
        self.len = len(array)
...
class LinearProbeHashTable(Generic[T]):
    ...
    def __init__(self, hash_base: int = DEFAULT_HASH_BASE, table_size: int = DEFAULT_TABLE_SIZE) -> None:
        ...
        self.next_prime = 0
        # following values are used to determine statistics
    ...
    def __rehash(self) -> None:
        ...
        # update stats values
        self.collision_count += new_hash.collision_count
        self.probe_total += new_hash.probe_total
        self.probe_max += new_hash.probe_max

    def __update_stats(self, probe_count:int)->None:
        """ updates the statistics based on probe count"""
        self.probe_total += probe_count
        if probe_count > 0:
            # if self.probe_total has increased then there was a collision
            self.collision_count += 1
            # determine the max probe
            if self.probe_max < probe_count:
    ...
    def __linear_probe(self, key: str, is_insert: bool) -> int:
        ...
        for _ in range(len(self.table)):  # start traversing
            if self.table[position] is None:  # found empty slot
                if is_insert:
                    self.__update_stats(probe_count)
                    return position
                else:
    ...
    def statistics(self) -> Tuple:
        """ returns a tuple (collision count, probe total, probe max, rehash count)
            consisting of:
                (a) the total number of collisions,
                (b) the sum of all the probe chain lengths,
    ...
    def __iter__(self):
        return HashIterator(self.table)
